...
由于lerobot_alohamini是AlohaMini的配套软件，所以内置了AlohaMini相关的配置文件，请注意：
- 如果您使用的是AlohaMini Solo（1主1从），请替换教程中的am_xxx.yaml为am_solo.yaml
- 如果您使用的是AlohaMini Duet（2主2从），请替换教程中的am_xxx.yaml为am_duet.yaml
...
AlohaMini Solo的默认插入顺序：
- 引导臂（Leader），对应ACM0
- 跟随臂(Follower)，对应ACM1

AlohaMini Duet的默认插入顺序：
...
如果不按此顺序插入USB接口，需要手工修改lerobot/configs/robot/am_xxx.yaml文件，改为相应的端口号。
...
#### 2. 修改configs/robot/am_xxx.yaml，配置为正确的摄像头序号
...
- 方式1：使用出厂自带的校准文件（推荐）：，直接将.cache/calibration/am_solo_bk文件夹改名为am_solo即可
...
python lerobot/scripts/control_robot.py calibrate \
    --robot-path lerobot/configs/robot/am_xxx.yaml
```
...
python lerobot/scripts/control_robot.py teleoperate \
    --robot-path lerobot/configs/robot/am_xxx.yaml
```
我们将看到摄像头窗体，并且Leader臂和Follower臂可以联动了
